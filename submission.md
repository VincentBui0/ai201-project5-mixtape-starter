...
Most of the AI use was environment troubleshooting: a venv mismatch pulling in an unrelated project's Python, PowerShell vs. Git Bash differences for curl and env vars, and one case where I left literal placeholder text like `<playlist_id_from_above>` in a URL instead of a real UUID. The error message made that one obvious once pointed out.

For orientation, I had the AI read `models.py`, `app.py`, and the route and service files and summarize what each was doing, then asked it to trace the add-to-playlist data flow specifically. I checked this against the actual files afterward and it matched. It caught something I'd have missed on a first pass: the playlist mutation and its notification are two separate commits with no shared transaction.

For each bug, I ran the failing test first, then had the AI help me read the suspicious function before deciding what to change. For bug #1 that meant working through what `weekday() != 6` was doing inside `update_listening_streak`. For bug #5 it meant isolating the `songs[:-1]` line in `get_playlist_songs`. For bug #4, the hypothesis that `rate_song` never calls `create_notification` came from reading `rate_song` and `add_to_playlist` side by side, not from guessing off the bug title.

The AI got one thing wrong along the way. I found a sixth bug by accident: `add_to_playlist` throws `IntegrityError: NOT NULL constraint failed: playlist_entries.position` when called through the HTTP route. The AI predicted that failure correctly from reading the code before I hit it live. But it also guessed that a dead import of `get_playlist_songs` inside `add_to_playlist` might be doing position math. I checked the function body myself and that import isn't used anywhere in it. AI reads code fast but doesn't always confirm a line actually executes before reasoning about it.

The AI proposed a fix for bug #4 that notifies the sharer on every re-rate, not just the first one. I tested that myself by rating the same song twice and watching the notification count. I kept the behavior since the issue doesn't specify which is correct, and wrote up the trade-off in that bug's entry instead of picking one silently.
...
The assertion error named `update_listening_streak`, so I opened `streak_service.py` and read the whole function, since it's short enough to take in at once rather than jumping to a guess. Every other branch in the function checks only the day gap since the last listen: same day, one day, more than one day. The increment branch had a second condition tacked onto the day-gap check, and nothing else in the file referenced weekday at all. That mismatch, one branch doing something none of the others did, is what told me I'd found it rather than just a suspicious spot.

The root cause: `update_listening_streak()` increments with `elif days_since_last == 1 and today.weekday() != 6:`. That `weekday() != 6` clause forces a reset to 1 any time the current listen lands on a Sunday, even when the previous listen was exactly one day before. The streak rules in the docstring don't mention any day-of-week exception. The clause shouldn't have been there.
...
Both failing tests led into `get_playlist_songs()` in `playlist_service.py`. I read the function from the top: it queries `playlist_entries` ordered by `position`, loops through building a list of song dicts, and returns. The return statement was the only place doing anything past that loop, so I checked it first rather than the query or the loop body. It was slicing the finished list with `[:-1]`. One line accounted for both symptoms at once, the wrong count and the missing final title, which is what confirmed it rather than left me guessing between the query and the slice.

The root cause: `get_playlist_songs()` returns `songs[:-1]` instead of `songs`, dropping the last song regardless of playlist length.

I changed the return to `songs`. Commit `f97142a`. Before committing I grepped for other callers to check for hidden dependencies on the truncated list. There's one real caller (`routes/playlists.py`) and one dead import inside `notification_service.py` that's never invoked in the function it sits in, so nothing else relied on the old behavior. All three playlist tests pass now, including `test_empty_playlist_returns_empty_list`, confirming the fix doesn't break the zero-song case.

The single-song case is covered by `test_playlist_with_single_song_returns_it`, added as a regression test. Before the fix, `[song][:-1]` on a one-item list returns `[]`, so any playlist with exactly one song would have come back empty.
...
I'd planned to reproduce the working path (`add_to_playlist`) live through the HTTP route for contrast, but that call throws a 500, the sixth bug described below. Instead I opened `notification_service.py` and read `add_to_playlist()` and `rate_song()` one after the other, since they're the two functions in the file that should behave the same way toward the notification system. `add_to_playlist()` calls `create_notification()` right after committing the playlist change, gated on the adder not being the sharer. `rate_song()` saves the `Rating` row, commits, and returns. No equivalent call anywhere in it. Seeing the two functions stacked against each other, one with the call and one without, is what made this a confirmed cause rather than an assumption from the bug title.
...
I fixed #1, #4, and #5 as the required three, each with a specific line-level root cause confirmed before touching any code. #2 (stale feed) and #3 (duplicate search results) are left as stretch goals.